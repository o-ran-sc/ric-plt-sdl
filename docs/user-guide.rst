...
:ref:`SDL deleloper guide <building_sdl_api_doc>`.
...
SDL is available in O-RAN-SC PackageCloud.io repository.
...
Redis server with IP address *10.20.30.40* and port *23541* is used::

   export DBAAS_SERVICE_HOST=10.20.30.40
   export DBAAS_SERVICE_PORT=23541

An example how environment variables can be set in bash shell, when Redis
sentinel name *my-master-sentinel* with IP address *10.20.30.50* and port
*23550* is used::

   export DBAAS_MASTER_NAME=my-master-sentinel
   export DBAAS_SERVICE_HOST=10.20.30.50
