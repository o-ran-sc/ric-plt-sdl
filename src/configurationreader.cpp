...
namespace
{
    ...
    void parseDatabaseConfigurationFromEnvironmentVariable(DatabaseConfiguration& databaseConfiguration,
                                                           const std::string& envStr,
                                                           const std::string& sourceName)
    {
        try
        {
            auto splitPoint(envStr.find(':'));

            validateAndSetDbType((splitPoint != std::string::npos) ? envStr.substr(0, splitPoint) : "", databaseConfiguration, sourceName);
            parseDatabaseServersConfigurationFromString(databaseConfiguration, (splitPoint != std::string::npos) ? envStr.substr(splitPoint+1): "", sourceName);
        }
        catch(const std::exception& e)
        {
            logErrorOnce(std::string(e.what()));
            throw;
        }
    }
...
ConfigurationReader::ConfigurationReader(const Directories& directories,
    ...
    configurationVariableName(DBCONF_VAR_NAME),
    configurationVariableValue({}),
    jsonDatabaseConfiguration(boost::none),
    logger(logger)
{
    auto envStr = system.getenv(configurationVariableName.c_str());
    if (envStr)
    {
        configurationVariableValue = envStr;
        sourceForDatabaseConfiguration = configurationVariableName;
    }
...
void ConfigurationReader::readConfiguration(T& input, const std::string& currentSourceName)
{
    ...
    if (sourceForDatabaseConfiguration != configurationVariableName)
    {
...
void ConfigurationReader::readDatabaseConfiguration(DatabaseConfiguration& databaseConfiguration)
{
    ...
    {
        if (sourceForDatabaseConfiguration == configurationVariableName)
            parseDatabaseConfigurationFromEnvironmentVariable(databaseConfiguration, configurationVariableValue, sourceForDatabaseConfiguration);
        else
